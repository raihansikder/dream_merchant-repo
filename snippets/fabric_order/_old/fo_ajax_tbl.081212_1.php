...
<div style="background-color:#FFC; margin-top:10px;">
  ...
  <table id="fab_avg_cons_table_<?php echo $fabric; ?>" border="0" cellpadding="0" cellspacing="0" style="display:none;">
    <tr>
      <td><?php echo $fabric;?> Average Consumption/Dozen: </td>
      ...
      <td><?php echo $fabric;?> Dia: </td>
      <td><input name="fo_avg_dia[]" id="<?php echo "fo_avg_dia_".$fabric;?>" type="text"  size="8" maxlength="8" value="" class="validate[required,custom[number]]" readonly="readonly"/></td>
    </tr>
  </table>
  <div class="clear"></div>
  <table>
    <tr style="vertical-align:top;">
      ...
      <td><span style='float:left;'>Fabrict type:<br/>
        </span></td>
      <td><span style='float:left;'>Fabrict Composition:<br/>
        </span></td>
      <td><span style='float:left;'>GSM:<br/>
        </span></td>
      <td><span style='float:left;'>Dia Unit:<br/>
        </span></td>
      <td><span style='float:left;'>Dia Form:<br/>
        </span></td>
      <td><span style='float:left;'>Remark:<br/>
        </span></td>
    </tr>
    <tr style="vertical-align:top;">
...
?>
    <tr>
      <td><!-- Hidden form values : start -->
        ...
        <input size="20" maxlength="40" name="placeholder"  value="" disabled="disabled"  /></td>
...
$('select[name=cal_type][id=<?php echo $fabric; ?>]').change(function(){
	...
	if(fo_fab_cal_type=='Size wise'){		
		...
		$("input[id=<?php echo "fo_avg_dia_".$fabric;?>]").val('');
		$("input[id=<?php echo "fo_avg_dia_".$fabric;?>]").attr('readonly','readonly');
		$("input[class=fod][id=<?php echo $fabric; ?>]").removeAttr('readonly').val('');
		$("table[id=fab_avg_cons_table_<?php echo $fabric; ?>]").hide();
		...
		$("input[id=<?php echo "fo_avg_dia_".$fabric;?>]").removeAttr('readonly');
		$("input[class=fod][id=<?php echo $fabric; ?>]").attr('readonly','readonly');
	}
});
...
$("input[id=<?php echo "fo_avg_dia_".$fabric;?>]").change(function(){
	var fo_avg_dia=$(this).val();
	//alert(fo_avg_consperdoz);	
	$("input[class=fod][id=<?php echo $fabric; ?>]").val(fo_avg_dia);
});
